  ...
  >
    ...
    >
      <div class="flex items-center justify-between px-6 py-4">
        <span class="text-navy font-semibold text-sm"
          >글로벌캠 자유게시판 검색</span
        >
        <span class="cursor-pointer" @click="modalCancel">
          <svg
      ...
      <div class="px-6 pt-1 pb-5">
        <input
          ...
          class="border border-gray-200 rounded text-sm px-4 py-3"
          @input="changeSearch"
        />
      </div>
      <div class="px-6 pb-4">
...
export default {
  ...
  methods: {
    modalCancel() {
      this.$parent.openSearchModal()
    },
    changeSearch(e) {
      this.search = e.target.value
    },
    formSubmit() {
      this.$router.push(`/globalfree/search/${this.search}`)
    },
  },
}
</script>
