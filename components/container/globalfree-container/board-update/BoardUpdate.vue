<template>
  ...
  >
    <div class="font-semibold text-navy text-sm px-7 py-5">
      글로벌캠 자유게시판 글쓰기
    </div>
    <div class="px-6 pt-1">
      <input
...
export default {
  ...
  async fetch() {
    await this.$store.dispatch('globalfree/getPostDetail', this.id).then(() => {
      this.title = this.$store.state.globalfree.postDetail.title
      this.content = this.$store.state.globalfree.postDetail.content
    })
  },
  methods: {
    ...
    async formSubmit() {
      await this.$client
        .patch(`/api/globalfree/${this.id}/update`, {
          title: this.title,
          content: this.content,
        })
        .then(() => {
          this.$toast.success('글을 수정했습니다.', { timeout: 3000 })
          this.$router.push(`/globalfree/${this.id}`)
        })
    },
  },
}
