...
export default {
  ...
  async fetch() {
    await this.$store
      .dispatch('globalfree/getCommentList', this.postId)
      .then(() => {
        this.comments = this.$store.state.globalfree.comments
        this.replys = this.$store.state.globalfree.replys
  ...
  methods: {
    ...
    async formSubmit() {
      await this.$client
        .post(`/api/globalfree/${this.postId}/comments/create`, {
    ...
    async deleteComment(commentId) {
      await this.$client
        .patch(`/api/globalfree/${this.postId}/comments/${commentId}/delete`)
        .then(() => {
    ...
    async thumbsUp(commentId) {
      try {
        await this.$client
          .post(`/api/globalfree/${this.postId}/comments/${commentId}/thumbs_up`)
          .then(() => {
            this.$fetch()
          })
      } catch (error) {
